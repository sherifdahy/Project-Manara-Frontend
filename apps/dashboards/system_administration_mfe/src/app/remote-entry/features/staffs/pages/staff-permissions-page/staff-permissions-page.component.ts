...
export class StaffPermissionsPageComponent implements OnInit {
  ...
  private loadData(): void {
    this.data$ = this.ps.getUserPermissions(this.universityUserId).pipe(
      tap((parsed) => {
  ...
  save(): void {
    this.ps
      .updateForUser(this.universityUserId, this.defaults, this.selected)
      .subscribe({
        next: () => {
